...
class LLMInfer:
    ...
    async def stream_chat_request(
        ...
        对连接级网络错误自动重试（最多 max_retries 次），一旦开始接收流数据则不再重试。
        ...
        while True:
            ...
            try:
                async for content, tool_calls, usage in self._attempt_stream(
                    ...
                    counter={"content": total_content_chars, "chunks": chunk_count, "first_chunk": first_chunk_time},
                ):
                    # 实时累加统计（counter 是引用，_attempt_stream 内部也会更新）
                ...
                if attempt > max_retries:
                    self._debug_log(f"重试已达上限({max_retries})，抛出错误: {type(e).__name__}: {e}")
                    raise
                wait = min(0.5 * attempt, 2.0)
                self._debug_log(f"网络错误 {type(e).__name__}，第 {attempt} 次重试，等待 {wait}s")
    ...
    async def _attempt_stream(
        self, messages, api_key, api_url, tools, model, max_tokens,
        temperature, top_p, frequency_penalty, presence_penalty,
        thinking_enabled, counter,
    ):
        ...
        async with client.stream(
            ...
            self._debug_log(f"数据接收完成 | 接收耗时: {data_receive_elapsed:.2f}ms")
